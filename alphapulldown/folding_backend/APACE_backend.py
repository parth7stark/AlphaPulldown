""" Implements structure prediction backend using APACE.
    Uses ray for GPU optimization
...
def predict_one_structure(
    ...
    """Ray task for predicting a single structure on a dedicated GPU."""
    ...
    logging.info(f"prediction costs (includes compilation time, see --benchmark) : {t_diff} s")

    # Add benchmark timing code here
    if benchmark:
        t_0 = time.time()
        ...
        logging.info(f"prediction costs (excludes compilation time) : {t_diff} s")
    ...
    # Update result with sequences and protein
    ...
    # will do this after collecting all the local results
...
def relax_single_structure(
    ...
    t_diff = end_time - t_0
    model_timings = {
        f'relax_{model_name}': t_diff,
        f'relax_{model_name}_start_time' : datetime.datetime.fromtimestamp(t_0).strftime("%H:%M:%S"),
        f'relax_{model_name}_end_time' : datetime.datetime.fromtimestamp(end_time).strftime("%H:%M:%S")
    }
    
    # Save the relaxed PDB locally
    relaxed_output_path = os.path.join(output_dir, f'relaxed_{model_name}.pdb')
    with open(relaxed_output_path, 'w') as f:
        f.write(relaxed_pdb_str)
    
    return model_name, relaxed_pdb_str, model_relax_metrics, model_timings


class APACEBackend(FoldingBackend):
    """
    ...
    def predict_individual_job(
        ...
        **kwargs,
        ...
        Parth - By job it means individual protein (monomer or muiltimer fasta)
        
        refer sbatch job array
        Job 1 - protein complex 1
        job 2 - protein complex 2
        ...
        **kwargs : dict
        ...
        timings = {}
        prediction_results = {}
        START = 0 #Tracks how many models have already completed predictions.
        ...
        if allow_resume:
            ...
            # Iterate over model runners to check for existing results:
            for model_index, (model_name, model_runner) in enumerate(model_runners.items()):
                unrelaxed_pdb_path = os.path.join(output_dir, f"unrelaxed_{model_name}.pdb")
                result_output_path = os.path.join(output_dir, f"result_{model_name}.pkl")
                ...
                if os.path.exists(unrelaxed_pdb_path) and os.path.exists(result_output_path):
                    # If both output files exist, it assumes the prediction was already completed.
                    # Loads the existing prediction results from the pickle file.
                    result_output_path = os.path.join(output_dir, f"result_{model_name}.pkl")
        ...
        """
        add ray predict_one_structure() here
        """
        # allocate 1 gpu to each model
        # Store feature_dict in Ray's object store
        feature_dict_ref = ray.put(original_feature_dict)

        # If model_runner is large, also consider:
        model_runner_refs = {name: ray.put(runner) for name, runner in model_runners.items()}
        # Launch Ray tasks for models that need to be predicted
        ray_tasks = []
        for model_index, (model_name, model_runner) in enumerate(model_runners.items()):
            if model_index < START:
                continue
                
            # Get the model runner reference from the object store
            model_runner_ref = model_runner_refs[model_name]
            # Launch Ray task for this model
            ray_tasks.append(
                predict_one_structure.remote(
                    model_name,
                    model_runner_ref,
                    model_index,
                    feature_dict_ref,
        ...
        # Process results
        for model_name, prediction_result, model_timings in ray_results:
            prediction_results[model_name] = prediction_result
    ...
    def predict(model_runners: Dict,
        ...
        # Iterates through a list of protein objects needing predictions. (protein_list.txt)
        for m in objects_to_model:
            object_to_model, output_dir = next(iter(m.items()))
            prediction_results = APACEBackend.predict_individual_job(
    ...
    def postprocess(
        ...
        **kwargs : dict
        ...
        for model_name, prediction_result in prediction_results.items():
            prediction_result.update(APACEBackend.recalculate_confidence(prediction_result,multimer_mode,
                                                                         total_num_res))
        ...
        if models_to_relax == ModelsToRelax.BEST:
            ...
            logging.warning(f"Unexpected value for models_to_relax: {models_to_relax}. No models will be relaxed.")
            to_relax = []  # Fallback to an empty list
        ...
        # Process results from Ray tasks
        for model_name, relaxed_pdb_str, model_relax_metrics, model_timings in ray_results:
            if relaxed_pdb_str is not None:
                relaxed_pdbs[model_name] = relaxed_pdb_str
                relax_metrics.update(model_relax_metrics)
                timings.update(model_timings)
        
        # Save combined relax metrics
        relax_metrics_path = os.path.join(output_dir, 'relax_metrics.json')
